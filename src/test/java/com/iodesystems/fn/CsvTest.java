...
public class CsvTest {
  ...
  public void testLargeCsv() throws IOException {
    ...
    assertEquals((Integer) (rows * cells), combine);
    pipedData.close();
  }
  ...
  public void testEscapes() throws IOException {
    ...
    assertEquals(Fn.list(Fn.list("\\\\", "\\", "\""), Fn.list("\\\\", "\\", "\"")), data.list());
    assertEquals(data.list(), Fn.fromCsv(Fn.toCsv(data)).list());
  }
  ...
  public void strangeField() throws IOException {
    ...
    assertEquals(Fn.list(Fn.list("", ","), Fn.list("a", "")), parsed.list());

    assertEquals(parsed.list(), Fn.fromCsv(Fn.toCsv(parsed)).toList());
  }
