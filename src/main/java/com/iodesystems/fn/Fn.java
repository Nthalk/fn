...
public class Fn<A> implements Iterable<A> {
  ...
  public static <A, B> Iterable<B> multiply(
      final Iterable<A> source, final From<A, Iterable<B>> multiplier) {
  ...
  public static <A> Fn<Option<A>> tails(Iterable<Iterable<A>> items) {
    return of(items).convert(Fn::last);
  }
  ...
  public String join(String glue) {
    ...
    if (out.length() > 0) {
      out.delete(out.length() - glue.length(), out.length());
    }
    return out.toString();
  }
  ...
  public Fn<A> intersection(Iterable<A> other) {
    ...
    return Fn.of(other).filter(set::contains);
  }
  ...
  public <B, C> Fn<Pair<A, C>> extractMatch(
      final From<A, B> extract, Iterable<C> against, From<C, B> extractAgainst) {
